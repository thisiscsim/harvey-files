...
// Broader set of keywords that suggest a review table would be beneficial,
// including softer signals that the existing review detection might miss.
const nudgeKeywords: KeywordWeight[] = [
  ...reviewKeywords,
  { word: 'research', weight: 0.9, variants: ['researching', 'researched'] },
  { word: 'identify', weight: 0.85, variants: ['identifying', 'identified', 'identification'] },
  { word: 'highlight', weight: 0.85, variants: ['highlighting', 'highlighted', 'highlights'] },
  { word: 'flag', weight: 0.8, variants: ['flagging', 'flagged', 'flags'] },
  { word: 'assess', weight: 0.8, variants: ['assessing', 'assessed', 'assessment'] },
  { word: 'evaluate', weight: 0.8, variants: ['evaluating', 'evaluated', 'evaluation'] },
  { word: 'audit', weight: 0.85, variants: ['auditing', 'audited', 'audits'] },
  { word: 'scan', weight: 0.75, variants: ['scanning', 'scanned'] },
  { word: 'check', weight: 0.7, variants: ['checking', 'checked', 'checks'] },
  { word: 'find', weight: 0.7, variants: ['finding', 'findings', 'found'] },
  { word: 'across', weight: 0.6, variants: [] },
  { word: 'documents', weight: 0.6, variants: ['files', 'contracts', 'agreements'] },
  { word: 'provisions', weight: 0.7, variants: ['clauses', 'terms', 'conditions'] },
  { word: 'diligence', weight: 0.9, variants: [] },
  { word: 'inventory', weight: 0.8, variants: ['inventorying'] },
  { word: 'catalog', weight: 0.8, variants: ['cataloging', 'catalogue', 'cataloguing'] },
];

/**
 * Determine whether the review table nudge should be shown.
 * Requires more than 10 files attached AND a review-like query.
 * Files are necessary because review tables operate across multiple documents.
 */
export function shouldShowReviewNudge(
  message: string,
  ...
  const nudgeScore = calculateArtifactScore(message, nudgeKeywords);
  const draftScore = calculateArtifactScore(message, draftKeywords);

  const nudgeThreshold = 0.6;

  if (process.env.NODE_ENV === 'development') {
    console.log('Nudge Detection:', { message, nudgeScore, draftScore, nudgeThreshold, fileCount });
  }

  return nudgeScore >= nudgeThreshold && nudgeScore >= draftScore;
}
